...
if __name__ == '__main__':
    ...
    ponzi_account = pd.read_csv("../Data/airdropHunter_account.txt", names=["account"])
    ponzi_account_set = set(ponzi_account.account.values)
    ...
    ratio = 1 - 30 / len(ponzi_account_set)
    print(ratio)
    ...
    if FLAGS.visual:
        ...
        X_phisher = np.array(y_hat_list)[p_idx]
        X_normal = np.array(y_hat_list)[n_idx]

        # Subsample normal samples for better visibility
        num_normal_samples = min(10000, len(X_normal))
        permutation = np.random.permutation(len(X_normal))
        X_normal_sample = X_normal[permutation[:num_normal_samples]]
        X4tsne = np.concatenate([X_normal_sample, X_phisher], axis=0)

        # Reshape to 2D array if not already
        if X4tsne.ndim > 1:
            X4tsne_reshaped = X4tsne
        else:
            X4tsne_reshaped = X4tsne.reshape(-1, 1)

        # Apply t-SNE
        tsne = TSNE(n_components=2, init="random")
        X_tsne = tsne.fit_transform(X4tsne_reshaped)

        # Plot t-SNE visualization
        plt.figure(figsize=(12, 8))

        # Scatter plot for normal samples
        plt.scatter(x=X_tsne[:num_normal_samples, 0], y=X_tsne[:num_normal_samples, 1], 
                    marker=".", label='Normal', alpha=0.5)

        # Scatter plot for phishing samples (in orange)
        plt.scatter(x=X_tsne[num_normal_samples:, 0], y=X_tsne[num_normal_samples:, 1], 
                    marker=".", color="orange", label='Phishing', alpha=0.5)
        ...
        plt.legend()
        plt.show()
        plt.savefig('ponzi_dnn_tsne.png')
